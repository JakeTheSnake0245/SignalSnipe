...
/* SS_UI_NAVBTN_PATCH */
  /* SS_UI_TOPBAR_WIFI_PATCH */
  .topbar{ display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap; }
  .topbar .btnbar{ display:flex; gap:10px; flex-wrap:wrap; }
...
<body>
  ...
  <div class="btnbar">
    ...
    <a class="navbtn" href="/wifi">WiFi Management</a>
  </div>
  <button type="button" class="danger" onclick="rebootConfirm()">Reboot Device</button>
</div>
...
def index():
    cfg = load_cfg()
    # SS_PATCH_DEFAULTS_V2
    # Defensive defaults so UI never 500s on missing optional fields
    if not isinstance(cfg, dict):
        cfg = {}
    cfg.setdefault('device', {})
    if not isinstance(cfg.get('device'), dict):
        cfg['device'] = {}
    cfg['device'].setdefault('name', 'signalsnipe')
    cfg['device'].setdefault('callsign', 'SignalSnipe')
    cfg['device'].setdefault('uid', 'signalsnipe')
    cfg.setdefault('location', {})
    if not isinstance(cfg.get('location'), dict):
        cfg['location'] = {}
    cfg['location'].setdefault('lat', 0.0)
    cfg['location'].setdefault('lon', 0.0)
    cfg['location'].setdefault('alt', 0.0)
    ranges = []
...
def save():
    ...
    # If MGRS provided, force mode so scanner converts it to lat/lon.
    # This prevents the common "mgrs saved but CoT stays at 0,0" failure.
    if str(cfg["location"].get("mgrs","")).strip():
        cfg["location"]["mode"] = "manual_mgrs"
    else:
        cfg["location"]["mode"] = "manual_latlon"
...
function rebootConfirm(){
  if(!confirm("Reboot SignalSnipe device now?\n\nThis will disconnect the WebUI for ~30-90 seconds.")) return;
  fetch("/api/reboot", {method:"POST"}).then(()=> {
    alert("Reboot command sent. This page will go offline shortly.");
  }).catch(()=> {
    alert("Failed to send reboot command (check permissions / logs).");
  });
}
function pickSSID(v){
...
<head>
  ...
  <style>
    ...
    button:hover, a.navbtn:hover { background: rgba(255,255,255,.10); }
    .danger { background: rgba(255,92,92,.12); border-color: rgba(255,92,92,.25); }
    .danger:hover { border-color: rgba(255,92,92,.6); }
    .topbar{ display:flex; justify-content:space-between; align-items:center; gap:12px; flex-wrap:wrap; }
    .topbar .btnbar{ display:flex; gap:10px; flex-wrap:wrap; }
    ...
    .pill {
      ...
      border-radius:999px; color: var(--muted); background: rgba(255,255,255,.04); font-size:.82rem;
    }
  </style>
<script>
function rebootConfirm(){
...
<body>
  ...
  <h1>SignalSnipe</h1>
  <small>WiFi management (NetworkManager / nmcli). If this shows “nmcli not found”, install NetworkManager.</small>

  {% if msg %}
  <div class="card"><b>{{msg}}</b></div>
  {% endif %}

  <div class="card">
    <h2 style="margin:0 0 8px 0;">Current WiFi Status</h2>
    <div class="row">
      <div>
        <div class="mini">Device: <span class="pill">{{status.device or "n/a"}}</span></div>
        <div class="mini" style="margin-top:6px;">State: <span class="pill">{{status.state or "n/a"}}</span></div>
        <div class="mini" style="margin-top:6px;">SSID: <span class="pill">{{status.ssid or "n/a"}}</span></div>
      </div>
      <div>
        <div class="mini">IP: <span class="pill">{{status.ip or "n/a"}}</span></div>
        <div class="mini" style="margin-top:6px;">NM: <span class="pill">{{status.nm or "unknown"}}</span></div>
        <div class="mini" style="margin-top:6px;">Last scan: <span class="pill">{{status.scantime or "n/a"}}</span></div>
      </div>
    </div>
    <div style="margin-top:10px;" class="mini">{{status.note or ""}}</div>
  </div>

  <div class="card">
    <h2 style="margin:0 0 8px 0;">Connect / Disconnect</h2>
    <form method="post" action="/wifi/connect">
      <div class="row">
        ...
        <div>
          ...
          <input name="password" type="password" placeholder="Leave blank for open networks" />
        </div>
      </div>
      <div style="margin-top:12px; display:flex; gap:10px; flex-wrap:wrap;">
        <button type="submit">Connect</button>
        <button type="submit" class="danger" formaction="/wifi/disconnect" formmethod="post">Disconnect WiFi</button>
      </div>
    </form>
    <div class="mini" style="margin-top:10px;">
      Tip: click a network SSID below to auto-fill the SSID field.
    </div>
  </div>

  <div class="card">
    <h2 style="margin:0 0 8px 0;">Nearby Networks</h2>
    <div class="mini">Showing up to 25 results.</div>
    <table>
      <thead>
        <tr><th>SSID</th><th>Security</th><th>Signal</th><th>Action</th></tr>
      </thead>
      <tbody>
      {% for n in nets %}
        <tr>
          ...
          <td>{{n.security}}</td>
          <td>{{n.signal}}</td>
          <td><button type="button" onclick="pickSSID({{n.ssid|tojson}})">Use</button></td>
        </tr>
      {% endfor %}
      {% if not nets %}
        <tr><td colspan="4" class="mini">No scan results (or WiFi device not available).</td></tr>
      {% endif %}
      </tbody>
    </table>
  </div>
...
def _wifi_device():
    if not _which("nmcli"):
        return None
    rc,out,err = _run(["nmcli","-t","-f","DEVICE,TYPE,STATE","dev","status"], timeout=4)
    if rc != 0:
        return None
    for line in (out or "").splitlines():
        parts = line.split(":")
        if len(parts) >= 3:
            d,t,st = parts[0].strip(), parts[1].strip(), parts[2].strip()
            if t == "wifi":
                return d
    return None

def _wifi_status():
    st = {"device": None, "state": None, "ssid": None, "ip": None,
          "nm": ("present" if _which("nmcli") else "missing"),
          "scantime": None, "note": ""}
    dev = _wifi_device()
    st["device"] = dev
    if not dev:
        st["note"] = "No WiFi device detected (nmcli wifi device)."
        return st

    rc,out,err = _run(["nmcli","-t","-f","DEVICE,STATE,CONNECTION","dev","status"], timeout=4)
    if rc == 0:
        for line in (out or "").splitlines():
    ...
    rc,out,err = _run(["nmcli","-t","-f","ACTIVE,SSID,DEVICE","dev","wifi"], timeout=6)
    if rc == 0:
        for line in (out or "").splitlines():
            parts = line.split(":")
            if len(parts) >= 3 and parts[0].strip() == "yes" and parts[2].strip() == dev:
                st["ssid"] = parts[1].strip()
                break

    rc,out,err = _run(["bash","-lc", f"ip -4 addr show {dev} | sed -n 's/.* inet \\([0-9.]*\\)\\/.*/\\1/p' | head -n1"], timeout=3)
    if rc == 0:
        st["ip"] = (out or "").strip() or None
...
def _wifi_scan():
    ...
    if not dev:
        return nets
    _run(["nmcli","dev","wifi","rescan"], timeout=10)  # ignore result
    rc,out,err = _run(["nmcli","-t","-f","SSID,SECURITY,SIGNAL","dev","wifi","list"], timeout=12)
    if rc != 0:
        return nets
    seen=set()
    for line in (out or "").splitlines():
        ...
        if len(parts) >= 3:
            ...
            sec=(parts[1] or "").strip() or "open"
            sig=(parts[2] or "").strip()
            if not ssid:
                continue
            key=(ssid,sec,sig)
            if key in seen:
    ...
    def sigv(x):
        try: return int(x.get("signal") or 0)
        except Exception: return 0
    nets.sort(key=sigv, reverse=True)
    return nets[:25]
...
def wifi_page():
    ...
    status = _wifi_status()
    status["scantime"] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    nets = _wifi_scan()
    return render_template_string(WIFI_TPL, status=status, nets=nets, msg=msg)
...
def wifi_connect():
    ...
    if not ssid:
        return redirect(url_for("wifi_page", msg="SSID required."))
    if not _which("nmcli"):
        return redirect(url_for("wifi_page", msg="nmcli not found (NetworkManager not installed)."))
    dev = _wifi_device()
    if not dev:
        return redirect(url_for("wifi_page", msg="No WiFi device detected."))
    if pwd:
        cmd = ["nmcli","dev","wifi","connect", ssid, "password", pwd, "ifname", dev]
    else:
        cmd = ["nmcli","dev","wifi","connect", ssid, "ifname", dev]
    rc,out,err = _run(cmd, timeout=25)
    if rc == 0:
        return redirect(url_for("wifi_page", msg=f"Connected (requested) to: {ssid}"))
    msg = (err.strip() or out.strip() or "unknown error")
    return redirect(url_for("wifi_page", msg=f"Connect failed: rc={rc} {msg}"))
...
def wifi_disconnect():
    ...
    if not dev:
        return redirect(url_for("wifi_page", msg="No WiFi device detected."))
    rc,out,err = _run(["nmcli","dev","disconnect", dev], timeout=10)
    if rc == 0:
        return redirect(url_for("wifi_page", msg="WiFi disconnected."))
    msg = (err.strip() or out.strip() or "unknown error")
    return redirect(url_for("wifi_page", msg=f"Disconnect failed: rc={rc} {msg}"))

@app.route("/api/reboot", methods=["POST"])
def api_reboot():
    try:
        # Web runs as "signalsnipe"; use locked-down sudoers rule for reboot only
        p = subprocess.run(["sudo","-n","/bin/systemctl","reboot"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
        if p.returncode == 0:
            return jsonify({"ok": True, "msg": "reboot requested"})
        return jsonify({"ok": False, "rc": p.returncode, "stderr": (p.stderr or "").strip(), "stdout": (p.stdout or "").strip()}), 500
    except Exception as e:
        return jsonify({"ok": False, "err": repr(e)}), 500
